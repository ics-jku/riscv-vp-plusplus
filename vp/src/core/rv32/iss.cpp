...
// to save *cout* format setting, see *ISS_T<T_csr_table>::show*
...
#define VExt VExtension<ISS_T<T_csr_table>>
...
ISS_T<T_csr_table>::ISS_T(uxlen_t hart_id, bool use_E_base_isa)
    : v_ext(*this), systemc_name("Core-" + std::to_string(hart_id)) {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::exec_step() {
...
template <class T_csr_table>
uint64_t ISS_T<T_csr_table>::_compute_and_get_current_cycles() {
...
template <class T_csr_table>
bool ISS_T<T_csr_table>::is_invalid_csr_access(uxlen_t csr_addr, bool is_write) {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::validate_csr_counter_read_access_rights(uxlen_t addr) {
...
template <class T_csr_table>
uxlen_t ISS_T<T_csr_table>::get_csr_value(uxlen_t addr) {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::set_csr_value(uxlen_t addr, uxlen_t value) {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::init(instr_memory_if *instr_mem, data_memory_if *data_mem, clint_if *clint, uxlen_t entrypoint,
                              uxlen_t sp) {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::sys_exit() {
	shall_exit = true;
}

template <class T_csr_table>
unsigned ISS_T<T_csr_table>::get_syscall_register_index() {
...
template <class T_csr_table>
uint64_t ISS_T<T_csr_table>::read_register(unsigned idx) {
	return (uint32_t)regs.read(idx);  // NOTE: zero extend
}

template <class T_csr_table>
void ISS_T<T_csr_table>::write_register(unsigned idx, uint64_t value) {
...
template <class T_csr_table>
uint64_t ISS_T<T_csr_table>::get_progam_counter(void) {
	return pc;
}

template <class T_csr_table>
void ISS_T<T_csr_table>::block_on_wfi(bool block) {
	ignore_wfi = !block;
}

template <class T_csr_table>
CoreExecStatus ISS_T<T_csr_table>::get_status(void) {
	return status;
}

template <class T_csr_table>
void ISS_T<T_csr_table>::set_status(CoreExecStatus s) {
	status = s;
}

template <class T_csr_table>
void ISS_T<T_csr_table>::enable_debug(void) {
	debug_mode = true;
}

template <class T_csr_table>
void ISS_T<T_csr_table>::insert_breakpoint(uint64_t addr) {
	breakpoints.insert(addr);
}

template <class T_csr_table>
void ISS_T<T_csr_table>::remove_breakpoint(uint64_t addr) {
	breakpoints.erase(addr);
}

template <class T_csr_table>
uint64_t ISS_T<T_csr_table>::get_hart_id() {
	return csrs.mhartid.reg;
}

template <class T_csr_table>
std::vector<uint64_t> ISS_T<T_csr_table>::get_registers(void) {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::fp_finish_instr() {
	fp_set_dirty();
	fp_update_exception_flags();
}

template <class T_csr_table>
void ISS_T<T_csr_table>::fp_prepare_instr() {
	assert(softfloat_exceptionFlags == 0);
	fp_require_not_off();
}

template <class T_csr_table>
void ISS_T<T_csr_table>::fp_set_dirty() {
	csrs.mstatus.fields.sd = 1;
	csrs.mstatus.fields.fs = FS_DIRTY;
}

template <class T_csr_table>
void ISS_T<T_csr_table>::fp_update_exception_flags() {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::fp_setup_rm() {
	auto rm = instr.frm();
	if (rm == FRM_DYN)
...
template <class T_csr_table>
void ISS_T<T_csr_table>::fp_require_not_off() {
	if (csrs.mstatus.fields.fs == FS_OFF)
		RAISE_ILLEGAL_INSTRUCTION();
}

template <class T_csr_table>
void ISS_T<T_csr_table>::return_from_trap_handler(PrivilegeLevel return_mode) {
	switch (return_mode) {
		case MachineMode:
...
template <class T_csr_table>
void ISS_T<T_csr_table>::trigger_external_interrupt(PrivilegeLevel level) {
	if (trace)
...
template <class T_csr_table>
void ISS_T<T_csr_table>::clear_external_interrupt(PrivilegeLevel level) {
	if (trace)
...
template <class T_csr_table>
void ISS_T<T_csr_table>::trigger_timer_interrupt() {
	if (trace)
...
template <class T_csr_table>
void ISS_T<T_csr_table>::clear_timer_interrupt() {
	if (trace)
...
template <class T_csr_table>
void ISS_T<T_csr_table>::trigger_software_interrupt() {
	if (trace)
...
template <class T_csr_table>
void ISS_T<T_csr_table>::clear_software_interrupt() {
	if (trace)
...
template <class T_csr_table>
std::string ISS_T<T_csr_table>::name() {
	return this->systemc_name;
}

template <class T_csr_table>
PrivilegeLevel ISS_T<T_csr_table>::prepare_trap(SimulationTrap &e) {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::prepare_interrupt(const PendingInterrupts &e) {
	if (trace) {
...
template <class T_csr_table>
PendingInterrupts ISS_T<T_csr_table>::compute_pending_interrupts() {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::switch_to_trap_handler(PrivilegeLevel target_mode) {
	if (trace) {
...
template <class T_csr_table>
void ISS_T<T_csr_table>::performance_and_sync_update(Opcode::Mapping executed_op) {
	++total_num_instr;
...
/*
 * see NOTE RVxx.1 in iss.h
 * Instruct the compiler to build all methods for all concrete classes based on
 * the ISS_T template
 */
namespace rv32 {
/* stand-alone RV32 */
template class ISS_T<csr_table>;
/* base for derived nuclei_core */
template class ISS_T<nuclei_csr_table>;
}  // namespace rv32
