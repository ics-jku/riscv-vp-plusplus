...
uint8_t CAN::loadTxBuf(uint8_t no, uint8_t byte) {
	...
	switch (command.state) {
		case LoadTX::id:
			...
			txBuf[no].frame.f_fields.f_id.id[command.payload_ptr++] = byte;
		...
		case LoadTX::length:
			...
			txBuf[no].frame.f_fields.length = byte;
			break;
		case LoadTX::payload:
			// std::cout << "\t[CAN] PL : " << std::hex << unsigned(byte) << std::endl;
			txBuf[no].frame.f_fields.payload[command.payload_ptr++] = byte;
			if (command.payload_ptr == txBuf[no].frame.f_fields.length) {
				command = LoadTX();
				state = State::init;
			}
			break;
...
uint8_t CAN::sendTxBuf(uint8_t no, uint8_t) {
	...
	mcp2515_buf_to_id(frame.can_id, extended, txBuf[no].frame.f_fields.f_id.id);
	frame.can_dlc = txBuf[no].frame.f_fields.length;
	memcpy(frame.data, txBuf[no].frame.f_fields.payload, txBuf[no].frame.f_fields.length);
...
uint8_t CAN::readRxBuf(uint8_t no, uint8_t) {
	...
	uint8_t ret = rxBuf[no].frame.raw[payload_ptr++];
	if (payload_ptr > rxBuf[no].frame.f_fields.length + 4) {
		payload_ptr = 0;
		state = State::init;
...
void CAN::enqueueIncomingCanFrame(const struct can_frame& frame) {
	...
	for (unsigned i = 0; i < 2; i++) {
		if (!(status & (1 << i)))  // Correnspond to MCP_STAT_RXnIF registers
		{
			...
			mcp2515_id_to_buf(
			    frame.can_id,
			    rxBuf[i].frame.f_fields.f_id.id);  // FIXME: This will break if extended frame or stuff happens
			rxBuf[i].frame.f_fields.length = frame.can_dlc;
			memcpy(rxBuf[i].frame.f_fields.payload, frame.data, frame.can_dlc);
			status |= 1 << i;
			return;
		}
	}
