...
VirtualBusMember::VirtualBusMember(sc_module_name name, Initiator& virtual_bus_connector, hwitl::Address base_address)
	...
	if (cmd == tlm::TLM_WRITE_COMMAND) {
		...
		switch (response) {
			...
			case hwitl::ResponseStatus::Ack::not_mapped:
				trans.set_response_status(tlm_response_status::TLM_ADDRESS_ERROR_RESPONSE);
				cerr << "[virtual_bus_tlm_connector] Write Address " << showbase << hex << base_address + addr << dec
				     << " not mapped" << endl;
				break;
			default:
				trans.set_response_status(tlm_response_status::TLM_GENERIC_ERROR_RESPONSE);
				cerr << "[virtual_bus_tlm_connector] Write Error '" << static_cast<unsigned>(response)
				     << "' at address " << showbase << hex << base_address + addr << dec << endl;
		}
		delay += m_write_delay;
		...
		switch (response->getStatus().ack) {
			...
			case hwitl::ResponseStatus::Ack::not_mapped:
				trans.set_response_status(tlm_response_status::TLM_ADDRESS_ERROR_RESPONSE);
				cerr << "[virtual_bus_tlm_connector] Read Address " << showbase << hex << base_address + addr << dec
				     << " not mapped" << endl;
				break;
			default:
				trans.set_response_status(tlm_response_status::TLM_GENERIC_ERROR_RESPONSE);
				cerr << "[virtual_bus_tlm_connector] Read Error " << static_cast<unsigned>(response->getStatus().ack)
				     << " at address " << showbase << hex << base_address + addr << dec << endl;
		}
		if (unaligned) {
			assert(len <= sizeof(hwitl::Payload));  // should always be the case
