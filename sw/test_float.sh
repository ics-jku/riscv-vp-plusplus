...
cd ../..
# set ZFINX to 0
find vp -type f -name 'csr.h' -exec sed -i 's/#define ZFINX 1/#define ZFINX 0/' {} +
make  # rebuild the project with ZFINX=0
cd sw/basic-c-float  # go to the float test directory
make clean sim clean  # clean build and run simulation
find ../../vp -type f -name 'csr.h' -exec sed -i 's/#define ZFINX 0/#define ZFINX 1/' {} +  # set ZFINX back to 1